...
console.log(chalk.bgBlack(chalk.bold.redBright("🟢 Comience con el código de país de su número de WhatsApp, ejemplo: +713945400"))) 
process.exit(0)
}} else {
while (true) {
addNumber = await question(chalk.bgBlack(chalk.bold.greenBright(`🟢 Ingresa el número que sera bot\nPor ejemplo: +5153049920 `)))
...
sock.sendMessage(anu.id, { text: `${pickRandom(['[ NUEVO ADMINISTRADOR ]\n\n', 'Hey'])} @${name.split("@")[0]} ${pickRandom(['Ahora eres administrador del grupo 🥳', 'Felicidades ahora eres parte staff 🎉'])}\n\n🫵 Acción echa por : @${usuario.split("@")[0]}`, mentions: [...groupAdmins.map(v => v.id)], 
 contextInfo:{
...
let name = num
let usuario = anu.author
sock.sendMessage(anu.id, { text: `@${name.split("@")[0]} ${pickRandom(['Joder ya no eres admin 🥲', 'jajaja ya no eres administrador 🤣'])}\n\n🫵 Acción echa por : @${usuario.split("@")[0]}`,
 contextInfo:{
 mentionedJid:[num, usuario],
 "externalAdReply": {"showAdAttribution": true,
 "containsAutoReply": true,
